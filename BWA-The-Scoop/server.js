...
function createNewComment(endpoint, message){
  const response = {};

  if(message.body && message.body.comment && message.body.comment.articleId 
    && message.body.comment.username && message.body.comment.body
    && database.users[message.body.comment.username] && database.articles[message.body.comment.articleId])
    {
      const articleIds = message.body.comment.articleId;
      const user = message.body.comment.username;
      const text = message.body.comment.body;
      const commentIds = database.nextCommentId;
...
function updateComment(commentId, message){
  ...
  const response = {};
  const commentIds = commentId.split('/').filter(segment => segment)[1];

  if(!message.body){
    response.status = 400;
  }
  else if(!database.comments[commentIds]) {
    response.status = 404;
  }
  else if(!message.body.comment){
    response.status = 400;
  }
 
  else if(message.body.comment.username && message.body.comment.body){
      const user = message.body.comment.username;
      const text = message.body.comment.body;
      const commentIds = message.body.comment.id;
      const articleIds = message.body.comment.articleId;
...
function deleteComment(commentId){
  const commentIds = Number(commentId.split('/').filter(segment => segment)[1]);
  const savedComment = database.comments[commentIds];
  const response = {};
  if(!savedComment){
    response.status = 404;
  }
  else {
    const deleteFromUser = database.users.user.commentIds;
    const removeCommentFromArticleId = database.comments[commentIds].articleId;
    database.comments[commentIds] = null;
    deleteFromUser.forEach(element => {
      if(element === commentIds){
        deleteFromUser.splice(deleteFromUser.indexOf(element),1);
      }
    });
    const deleteFromArticle = database.articles[removeCommentFromArticleId].commentIds;
    deleteFromArticle.forEach(element => {
      if(element === commentIds){
        deleteFromArticle.splice(deleteFromArticle.indexOf(element),1);
      }
    });
    response.status = 204;    
  } 
...
function upvoteComment(commentId, username){
  const response = {};    
  if(Object.keys(username).length != 0 && commentId){
    const comment = commentId.split('/').filter(segment => segment)[1];
    const user = username.body.username;
    let savedComment = database.comments[comment]; 
    if(database.users[user] && database.comments[comment]){
      if(database.comments[comment].upvotedBy != user){
        const data = database.comments[comment].downvotedBy;        
          for(let i = 0; i < data.length; i++){
            if(data[i] === user){
              data.splice(data[i],1);
            }
          }
        //updating database and returning updated comment       
        database.comments[comment].upvotedBy.push(user);
...
function downvoteComment(commentId, username){
  ...
  if(Object.keys(username).length != 0 && commentId){
    ...
    let savedComment = database.comments[comment]; 
    if(database.users[user] && database.comments[comment]){
      if(database.comments[comment].downvotedBy != user){
        const data = database.comments[comment].upvotedBy;        
          for(let i = 0; i < data.length; i++){
            if(data[i] === user){
              data.splice(data[i],1);
            }
        ...
        //updating database and returning updated comment       
        database.comments[comment].downvotedBy.push(user);
        response.body = {comment: savedComment};
        response.status = 200;
        }
      }  
